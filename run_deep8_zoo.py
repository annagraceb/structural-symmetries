"""Train the 8-layer zoo (Phase 3). 2 seeds per freeze to fit time budget."""
...
from config_deep8 import (
    ModelConfigDeep8, TrainConfigDeep8,
    FREEZABLE_COMPONENTS_DEEP8, N_SEEDS_PER_FREEZE, N_BASELINE_SEEDS,
)
...
def main():
    ...
    for comp in FREEZABLE_COMPONENTS_DEEP8:
        for seed in range(N_SEEDS_PER_FREEZE):
            configs.append((comp, seed, comp))

    print(f"\nTraining {len(configs)} models (8 layers)...")
    summary = {"models": []}
    start = time.time()
    for i, (name, seed, component) in enumerate(configs):
        ...
        train_cfg = TrainConfigDeep8(seed=seed, frozen_component=component)
